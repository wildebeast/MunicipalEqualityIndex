...
def get_municipality_links_on_page(state_page, state_abbr):
    ...
    anchors = state_page.find_all('a', href=True)
    muni_links = []
    for a in anchors:
        href = a['href']
        # Normalize relative URLs to absolute form
        if href.startswith('/resources/municipalities/'):
            href = 'https://www.hrc.org' + href

        if not href.startswith('https://www.hrc.org/resources/municipalities/'):
            continue

        if '/search' in href:
            continue

        # Accept slug patterns like '-ca' or '-ca-3'
        pattern = r'-' + state_abbr.lower() + r'(?:-\d+)?$'
        if not re.search(pattern, href):
            continue

        muni_links.append(href)
...
def get_all_municipality_links():
    ...
    for state in states:
        ...
        for link in get_municipality_links_for_state(state_name, state_abbr):
            city_id += 1
            slug = link.split('/')[-1]
            # Remove state suffix and optional numeric tag
            slug = re.sub(r'-' + state_abbr.lower() + r'(?:-\d+)?$', '', slug)
...
def get_scores():
    ...
    for city in cities:
        ...
        for row, score in zip(template_copy, scores):
            ...
            # print(new_row)
            data.append(new_row)
    with open(f'mei-data-{YEAR}.csv', 'w') as f:
...
def get_score_totals_by_municipality(muni_data):
    ...
    with open(f'mei-totals-by-municipality-{YEAR}.csv', 'w') as f:
...
def main():
    ...
    if os.path.isfile(f'mei-data-{YEAR}.csv'):
        data = None
        print(f'File mei-data-{YEAR}.csv already exists, skipping.')
    else:
        data = get_scores()
    if os.path.isfile(f'mei-totals-by-municipality-{YEAR}.csv'):
        print(f'File mei-totals-by-municipality-{YEAR}.csv exists, skipping.')
    else:
        if data is None:
            with open(f'mei-data-{YEAR}.csv', 'r') as f:
